...
def read_local_value(key: str) -> str:
...
def write_local_value(key: str, new_value: str) -> None:
    pattern = re.compile(rf"^(\s*ARG\s+{re.escape(key)}=).+?(\s*)$")
...
def main() -> None:
    ...
    if os.environ.get("WRITE_UPSTREAM_VERSION") == "true" and updates_available:
        ...
        write_local_value(str(upstream.get("digest_key", "")).strip(), latest_digest)
